# -*- coding: utf-8 -*-
"""
Generates a sine-surface type noise pattern. Used to mimic the segmented LM cells.
...
R = 249 # Dish Radius (Aperture) in millimeter, with 1mm subtracted to prevent edge effect.
...
nPV = 0.1 # Peak to Vally Scale for Noise 1, units in mm

''' Sine Noise as Variation on Hölder table function '''

# Sinusoidal functions Noise (Mock Hex Noise)
nullx, nullz = np.meshgrid(x, z)
ny = np.zeros_like(nullx)

nPV = .001 # Peak to Vally Scale for Noise, units in mm. Not quite a 1:1 scalar for ny, so check after running.

freq   = .1 # Related to the width of the "cells"

ny     = -1 * nPV * abs( np.sin(freq*xx)*np.cos(freq*zz) * np.exp( abs( (R*freq - (np.sqrt((1*xx)**2 + (1*zz)**2))/np.pi))**.2 ) )
...
n_mat = {"nz1":Noise_Grid}
savemat("sineCellNoise.mat", n_mat)
...
plt.title('Psuedo Hölder Table Function')
plt.grid('on')
plt.xlabel('X (mm)')
plt.ylabel('Z (mm)')
plt.show()
