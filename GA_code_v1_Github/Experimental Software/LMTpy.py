...
class Instruments:
    ...
    def connect_all(self):
        ...
        try:
            ...
            self.camera.ExposureTime.Value = 20000
...
class Experiment(Instruments):
    ...
    @staticmethod
    def deg_to_pos(deg):
        val= round(-3.91557e-4*deg**3 - 9.33334e-3*deg**2 -2.3454*deg +30.26101,3)
        return val

    @staticmethod
    def pos_to_deg(pos):
        val = round(-8.1748e-4*pos**2 - 3.73873e-1*pos+12.07601,3) #polynomial fit of stage angle vs motor position (R-squared>0.999)
        return val
    ...
    def motor_scan_timed(self,DEGREES,DURATION=10,START_DELAY=5,IMG_DELAY=0.5, 
        ...
        for deg in DEGREES:
            ...
            plt.savefig(os.path.join(folder_path,'Stage angle %0.2f degrees.png'%deg))
            plt.close(fig2)
            with open(os.path.join(folder_path,'Stage angle %0.2f degrees.pickle'%deg),'wb') as f:
                pickle.dump(data,f)
